...
1. 国行 BRAVIA 8M2 使用 `clash-verge-tv-0.1.11-meta-armeabi-v7a-release.apk`。其他型号可先用 ADB 执行 `adb shell getprop ro.product.cpu.abilist`：包含 `arm64-v8a` 就选 64 位包，只包含 `armeabi-v7a` 就选 32 位包。
...
adb install -r clash-verge-tv-0.1.11-meta-armeabi-v7a-release.apk
```
